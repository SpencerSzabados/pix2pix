...
IMAGE_EXTENSIONS = {'bmp', 'jpg', 'jpeg', 'pgm', 'png', 'PNG', 'ppm',
                    'tif', 'tiff', 'webp', "JPEG"}
...
def calculate_eqv_fid_given_paths(path, batch_size, device, dims, img_size, num_workers, eqv='Z2'):
    ...
    m0, s0 = calculate_statistics_of_path_cache(path, gen_Z2_cache_file_dir, \
                                              model, batch_size, dims, img_size, device, num_workers, eqv='Z2')
    ...
    if eqv == 'V':
        ...
        m_, s_ = calculate_statistics_of_path_cache(path, gen_V_cache_file_dir, \
                                              model, batch_size, dims, img_size, device, num_workers, eqv='Z2', gen_op=gen_op)
    ...
    if eqv == 'C4':
        for k in range(1, 4):
            ...
            m_, s_ = calculate_statistics_of_path_cache(path, gen_cache_file_dir, \
                                                model, batch_size, dims, img_size, device, num_workers, eqv='Z2', 
                                                gen_op=lambda x: torch.rot90(x, k = k, dims=[-1, -2]))
            mean_var_list.append([m_, s_])
    ...
    if eqv == 'D4':
        for k in range(1, 4):
            ...
            m_, s_ = calculate_statistics_of_path_cache(path, gen_cache_file_dir, \
                                                model, batch_size, dims, img_size, device, num_workers, eqv='Z2', 
                                                gen_op=lambda x: torch.rot90(torch.flip(x, dim = [-2]), k = k, dims=[-1, -2]))
...
def main():
    ...
    fid_value = calculate_eqv_fid_given_paths(path=args.path,
                                          batch_size=args.batch_size,
                                          device=device,
                                          dims=args.dims,
                                          img_size=args.img_size,
